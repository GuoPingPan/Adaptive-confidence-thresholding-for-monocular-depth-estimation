...
class SegNet(nn.Module):
    ...
    def forward(self, input_img):
        ...
        # target_low, ch0 = convert_to_single_channel(depthresult)
        # bs, ch, h, w = guide.shape
        # hh = torch.arange(h, dtype=guide.dtype, device=guide.device)
        # ww = torch.arange(w, dtype=guide.dtype, device=guide.device)
        # guide = torch.cat([guide * self.scale_color,
...
class ParamNet(nn.Module):
    ...
    def forward(self, confidence_map, activations):
        ...
        x = torch.cat([x, activations[0]], dim=1)  # 64
        ...
        x = torch.cat([x, activations[1]], dim=1)  # 128
        ...
        x = torch.cat([x, activations[2]], dim=1)  # 256
        ...
        x = torch.cat([x, activations[3]], dim=1)  # 512
        ...
        # print(x.shape) #16, 512, 1, 1

        x = x.view(confidence_map.shape[0], 512)
        x = self.fc8(x)

        # x = torch.cat(x, activations[4]) #512

        # x = F.relu(self.fc6(x, 7 * 7 * 512))
...
class UncertainNet(nn.Module):
    ...
    def forward(self, input_img, indiceslist):
        ...
        # y_41d = F.relu(self.uncertain_convtr_41(y_42d))
        ...
        # y_31d = F.relu(self.uncertain_convtr_31(y_32d))
        ...
        # y_21d = F.relu(self.uncertain_convtr_21(y_22d))
        ...
        #y_00d = self.uncertain_convtr_00(y_01d)
...
"""class CCNN(nn.Module):
    ...
    def __init__(self, input_channels=1, output_channels=1):
        ...
        self.conv1 = nn.Conv2d(self.input_channels, filters, kernel_size=kernel_size, stride=1, padding=1)
        self.conv2 = nn.Conv2d(filters, filters, kernel_size=kernel_size, stride=1, padding=1)
        self.conv3 = nn.Conv2d(filters, filters, kernel_size=kernel_size, stride=1, padding=1)
        self.conv4 = nn.Conv2d(filters, filters, kernel_size=kernel_size, stride=1, padding=1)

        self.fc1 = nn.Conv2d(filters, fc_filters, kernel_size=1, stride=1, padding=0)
        self.fc2 = nn.Conv2d(fc_filters, fc_filters, kernel_size=1, stride=1, padding=0)
        #self.fc1 = nn.Linear(filters, fc_filters, bias=True)
        #self.fc2 = nn.Linear(fc_filters, fc_filters, bias=True)
        #self.prediction = nn.Linear(fc_filters, output_channels, bias=True)

        self.prediction = nn.Conv2d(fc_filters, output_channels, kernel_size=1, stride=1, padding=0)
        self.activation = nn.Sigmoid()

    def forward(self, disp):

        x = F.relu(self.conv1(disp))
...
class SegEncoder(nn.Module):
    ...
    def forward(self, input_img):
        ...
        return x_00d, activations, indiceslist
